...
def dashboard_scan() -> Response:
    ...
    try:
        response = build_scan_response(urls, parse_positive_int(request.form.get("max_posts", MAX_POSTS_DEFAULT), MAX_POSTS_DEFAULT), parse_bool(request.form.get("include_comments"), False), save=True, debug=True, accounts=settings.get("accounts", []))
...
<style>:root{--bg:#07111f;--p:#0f172a;--r:#111827;--t:#e5e7eb;--m:#94a3b8;--b:#263349;--a:#38bdf8;--g:#22c55e;--d:#fb7185;--v:#a78bfa}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:linear-gradient(180deg,#020617,#0f172a,#111827);color:var(--t)}a{color:#7dd3fc;text-decoration:none}.wrap{max-width:1440px;margin:auto;padding:24px}.hero,.panel{background:rgba(15,23,42,.94);border:1px solid var(--b);border-radius:20px;padding:20px;margin-bottom:18px}.meta,.muted{color:var(--m)}.stats,.form-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.stat{background:#1f2937;border:1px solid var(--b);border-radius:14px;padding:14px}.v{font-size:30px;font-weight:900}.filters,.form-row,.head,.actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.head{justify-content:space-between}label{display:grid;gap:6px;color:var(--m);font-size:14px}input,textarea{width:100%;background:#0b1220;color:var(--t);border:1px solid var(--b);border-radius:12px;padding:11px}textarea{min-height:104px}.btn{border:0;border-radius:12px;padding:10px 14px;min-height:42px;background:var(--a);color:#021024;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;justify-content:center}.btn.ghost{background:#1f2937;color:var(--t);border:1px solid var(--b)}.btn.copy{background:var(--g);color:#052e16}.btn.secondary{background:var(--v);color:#160b2f}.msg{padding:12px;border-radius:12px;border:1px solid var(--b);margin-bottom:16px;background:rgba(34,197,94,.13);color:#bbf7d0}.msg.error{background:rgba(251,113,133,.13);color:#fecdd3}.list{display:grid;gap:10px}.list-head,.row{display:grid;grid-template-columns:76px 105px 62px minmax(100px,.7fr) minmax(220px,1.5fr) minmax(180px,1fr) 150px 230px;gap:10px;align-items:center}.list-head{padding:0 12px;color:var(--m);font-size:12px;text-transform:uppercase}.row{background:rgba(17,24,39,.9);border:1px solid var(--b);border-radius:15px;padding:12px;min-width:0}.cell{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.preview{white-space:normal;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;line-height:1.35;max-height:2.7em}.pill{display:inline-flex;width:max-content;padding:4px 9px;border-radius:999px;background:rgba(56,189,248,.15);color:#7dd3fc;font-size:12px;font-weight:800}.saved{background:rgba(34,197,94,.15);color:#86efac}.rejected{background:rgba(251,113,133,.15);color:#fecdd3}.empty{padding:16px;border:1px dashed var(--b);border-radius:14px;color:var(--m)}.modal{display:none;position:fixed;inset:0;z-index:50;background:rgba(2,6,23,.78);padding:20px}.modal.open{display:grid;place-items:center}.box{width:min(980px,100%);max-height:92vh;overflow:auto;background:#0f172a;border:1px solid var(--b);border-radius:20px}.box-head{position:sticky;top:0;background:#0f172a;border-bottom:1px solid var(--b);padding:14px;display:flex;justify-content:space-between}.box-body{padding:14px;display:grid;gap:12px}.details{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.detail{background:#0b1220;border:1px solid var(--b);border-radius:12px;padding:10px;word-break:break-word}.k{font-size:12px;color:var(--m);text-transform:uppercase}pre{white-space:pre-wrap;word-break:break-word;background:#0b1220;border:1px solid var(--b);border-radius:12px;padding:12px}.hidden-json{display:none}@media(max-width:900px){.wrap{padding:14px}.form-grid{grid-template-columns:1fr}.list-head{display:none}.row{grid-template-columns:1fr}.cell{white-space:normal}.cell:before{content:attr(data-label);display:block;color:var(--m);font-size:11px;text-transform:uppercase;margin-bottom:3px}.actions{display:grid;grid-template-columns:repeat(3,1fr)}.btn{width:100%;padding:12px}.modal{padding:0}.box{height:100%;max-height:100vh;border-radius:0}.details{grid-template-columns:1fr}}</style></head><body><div class="wrap">
...
<div class="form-grid"><div class="panel"><h2>Manual Scan</h2><div class="meta">Paste one Facebook group UID, slug, or link per line.</div><form method="post" action="/dashboard/scan">{% if token %}<input type="hidden" name="token" value="{{ token }}">{% endif %}<label>Groups<textarea name="groups" placeholder="123456789012345&#10;vayvonkinhdoanh">{{ settings_groups_text }}</textarea></label><div class="form-row"><label>Max posts<input type="number" name="max_posts" min="1" max="1000" value="{{ settings.max_posts }}"></label><label><input type="checkbox" name="include_comments" value="1" {% if settings.include_comments %}checked{% endif %}> Include comments</label><button class="btn" type="submit">Scan &amp; Save</button></div></form></div><div class="panel"><h2>Scheduled Scan Settings</h2><div class="meta">Use Cloud Scheduler to POST /scheduled-scan with X-Review-Token.</div><form method="post" action="/dashboard/settings">{% if token %}<input type="hidden" name="token" value="{{ token }}">{% endif %}<label>Groups<textarea name="groups">{{ settings_groups_text }}</textarea></label><label>Time windows<textarea name="time_windows" placeholder="08:00-11:30&#10;13:30-17:00">{{ settings_time_windows_text }}</textarea></label><label>Accounts / Cookie Strings<textarea name="accounts" placeholder="c_user=...; xs=...; fr=...">{{ settings_accounts_text }}</textarea></label><div class="form-row"><label>Max posts<input type="number" name="max_posts" min="1" max="1000" value="{{ settings.max_posts }}"></label><label>Timezone<input type="text" name="timezone" value="{{ settings.timezone }}"></label><label><input type="checkbox" name="include_comments" value="1" {% if settings.include_comments %}checked{% endif %}> Include comments</label><label><input type="checkbox" name="enabled" value="1" {% if settings.enabled %}checked{% endif %}> Enabled</label><button class="btn secondary" type="submit">Save Settings</button></div></form></div></div>
{% macro render_list(title, desc, rows) %}<section class="panel"><div class="head"><div><h2>{{ title }}</h2><div class="meta">{{ desc }}</div></div><span class="pill">{{ rows|length }} item(s)</span></div>{% if rows %}<div class="list"><div class="list-head"><div>Saved</div><div>Category</div><div>Score</div><div>Author</div><div>Preview</div><div>Link</div><div>Created</div><div>Actions</div></div>{% for item in rows %}{% set link = item.display_url or item.post_url or item.source_url %}<div class="row"><div class="cell" data-label="Saved">{% if item.saved %}<span class="pill saved">true</span>{% else %}<span class="pill rejected">false</span>{% endif %}</div><div class="cell" data-label="Category"><span class="pill">{{ item.category or item.ai_category or item.intent }}</span></div><div class="cell" data-label="Score">{{ item.score }}</div><div class="cell" data-label="Author">{{ item.author or '-' }}</div><div class="cell preview" data-label="Preview">{{ item.preview or item.text }}</div><div class="cell" data-label="Link">{% if link %}<a href="{{ link }}" target="_blank" rel="noopener">{{ link }}</a>{% else %}-{% endif %}</div><div class="cell" data-label="Created">{{ item.created_at }}</div><div class="actions" data-label="Actions"><button class="btn ghost" type="button" onclick="openItemModal(this)">Xem</button><button class="btn copy" type="button" onclick="copyText({{ link|tojson }}, this)">Copy link</button><button class="btn copy" type="button" onclick="copyText({{ item.text|tojson }}, this)">Copy text</button></div><script type="application/json" class="hidden-json">{{ item|tojson }}</script></div>{% endfor %}</div>{% else %}<div class="empty">No items yet.</div>{% endif %}</section>{% endmacro %}
...
</div><div class="modal" id="itemModal"><div class="box"><div class="box-head"><strong>Item detail</strong><button class="btn ghost" onclick="closeItemModal()">Close</button></div><div class="box-body" id="modalBody"></div></div></div><script>async function copyText(t,b){try{await navigator.clipboard.writeText(t||'');let o=b.textContent;b.textContent='Copied';setTimeout(()=>b.textContent=o,1200)}catch(e){alert('Copy failed')}}function esc(v){if(Array.isArray(v))v=v.join(', ');if(v===true)v='true';if(v===false)v='false';v=v?String(v):'-';return v.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function detail(k,v){return `<div class="detail"><div class="k">${k}</div><div>${esc(v)}</div></div>`}function openItemModal(btn){let d=JSON.parse(btn.closest('.row').querySelector('.hidden-json').textContent);let fields=['saved','category','score','author','group_url','post_url','source_url','selector','reason','matched_keywords','suggested_comments','created_at'];document.getElementById('modalBody').innerHTML=`<div class="details">${fields.map(k=>detail(k,d[k]||d.ai_category||d.ai_reason)).join('')}</div><div class="detail"><div class="k">full text</div><pre>${esc(d.text||d.raw_text||'')}</pre></div>`;document.getElementById('itemModal').classList.add('open')}function closeItemModal(){document.getElementById('itemModal').classList.remove('open')}document.addEventListener('keydown',e=>{if(e.key==='Escape')closeItemModal()})</script></body></html>
"""
